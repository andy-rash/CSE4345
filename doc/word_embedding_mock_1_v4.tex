...
This paper outlines a few methods used in this repository to map all unique words found in a corpus to vectors in a vector space. The idea, and hope, is that some relationships between words found in the corpus will be preserved through this mapping and will manifest as characteristics of the word vectors. Successful implementations and strides in this area include \href{https://en.wikipedia.org/wiki/Latent_Semantic_Analysis}{Latent Semantic Analysis}, \href{https://en.wikipedia.org/wiki/Word2vec}{"Word2vec"}, and \href{https://nlp.stanford.edu/projects/glove/}{"GloVe"}. At its heart, the goal of word embedding is to quantify the conceptual relationships that exist within our vocabulary and to create a mathematical system to describe and analyze them. Abstractly word embedding seeks to build a sort of concept space where different regions and axes can represent ideas, and to embed word vectors inside this space where their lengths and directions place them in meaningful locations. Models search for word vectors whose distances apart represent essential relationships between them. The key idea (and assumption) that is exploited by these models is that the more frequently words appear together in natural language the more related these words are conceptually. Most methods exploit this property by first counting the cooccurrences of word pairs and then by building word vectors accordingly. Models search for vectors whose inner product or cosine similarity or euclidean distance apart can be traced back to this list of cooccurrences. Many different techniques have been employed to find these word vectors, but the power of these representations is probably best illustrated by a commonly sited example from Word2vec's results:
...
%
%	Eigen Decomposition of a forced Symmetric Positive Matrix
%
...
%
%	Singular Value Decomposition of a forced Symmetric Positive Definite Matrix
%
