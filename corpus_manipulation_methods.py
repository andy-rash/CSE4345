...
def djdx_new(x):
	...
	for i in range(0,n):
		if i != x:
			wj = W[:,i]
			#multiply by wj
			#new_wi += wj*(np.dot(wi, wj) - A[x,i])
			new_wi += np.multiply(wj,(np.dot(wi, wj) - A[x,i]))

	return new_wi #np.multiply(W[:,x],total_error)
...
for i in range(iterations):
	...
	W[:,x] = W[:,x] - learning_rate * djdx_new(x)
...
"""
come up with system to test how well it works

-need to be able to view which words the coocurence matrix thinks are most similar
-need to be able to view which words the vector space thinks are most similar
-disimilar as well

-need method to view effectivness of training parameters as well as methods involing diffrent functions
	-biases, adaptive learning etc.
"""
